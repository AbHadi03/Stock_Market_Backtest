...
with st.expander("📖 Strategy Description & Rules (Hindi/English)"):
    st.markdown("""
    यह रहा आपकी "Paisa Double Strategy" का एक विस्तृत और प्रोफेशनल दस्तावेज़ (Document)। इसे आप सेव करके रख सकते हैं, यह आपके डैशबोर्ड के हर एक विकल्प (Optional Box) को गहराई से समझाता है:

    📄 मास्टर समरी: Paisa Double Strategy (Spot Martingale)
    यह रणनीति गणितीय बढ़त और समय-आधारित होल्डिंग (CDSL Rule) का एक अनूठा मिश्रण है, जिसे विशेष रूप से स्टॉक मार्केट में सुरक्षित रिकवरी और मुनाफे के लिए बनाया गया है।

    ⚙️ डैशबोर्ड पैरामीटर्स और ऑप्शंस (Optional Boxes)
    डैशबोर्ड में दिए गए हर बॉक्स का उपयोग आप अपनी रिस्क लेने की क्षमता के अनुसार कर सकते हैं:

    Initial Investment (शुरुआती पूंजी):
    - यह वह फिक्स्ड राशि है जिससे आप अपने ट्रेडिंग चक्र (Cycle) की शुरुआत करते हैं।
    - हर सफल 'Take Profit' के बाद सिस्टम वापस इसी राशि पर लौट आता है।

    Take Profit % (मुनाफा लक्ष्य):
    - रेंज: आप इसे 4% से 6% के बीच चुन सकते हैं।
    - विवरण: 4% रखने पर आपको जल्दी-जल्दी एग्जिट मिलते हैं, जबकि 6% रखने पर प्रति ट्रेड मुनाफा बढ़ जाता है।

    Stop Loss % (नुकसान की सीमा):
    - रेंज: इसे 2% से 3% के बीच सेट किया जा सकता है।
    - विवरण: यह वह स्तर है जहाँ आप छोटा नुकसान स्वीकार करके अगले ट्रेड में रिकवरी (2x निवेश) की तैयारी करते हैं।

    Subsequent Multiplier (निवेश गुणक):
    - डिफ़ॉल्ट: 2.00 (2x)।
    - विवरण: यदि पिछला ट्रेड स्टॉप लॉस पर कटा है, तो अगला निवेश इस गुणक से बढ़ जाएगा (जैसे ₹10,000 → ₹20,000)।

    📑 मुख्य ट्रेडिंग नियम (SOP)
    1. **एंट्री**: स्टॉक को बाज़ार बंद होने (3:25 PM के आसपास) के भाव पर खरीदें।
    2. **CDSL सुरक्षा नियम**: शेयर खरीदने के अगले दिन (T+1) कोई कार्रवाई न करें। केवल दूसरे दिन से ही TP/SL के लिए चेक करें।
    3. **निकास (Exit)**:
        - **जीत (Win)**: यदि कीमत TP% हिट करे → मुनाफा बुक करें और निवेश को Initial Amount पर रीसेट करें।
        - **हार (Loss)**: यदि कीमत SL% हिट करे → ट्रेड काटें और अगले ट्रेड के लिए निवेश को Multiplier (2x) से बढ़ाएं।
    """)
...
if 'p3_start_date' not in st.session_state:
    st.session_state.p3_start_date = datetime.today().date() - timedelta(days=365)
if 'p3_end_date' not in st.session_state:
    st.session_state.p3_end_date = datetime.today().date()

with col_s1:
    START_DATE = st.date_input("Start Date", key="p3_start_date")
with col_s2:
    END_DATE = st.date_input("End Date", key="p3_end_date", max_value=datetime.today().date())
