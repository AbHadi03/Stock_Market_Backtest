...
if 'p4_start_date' not in st.session_state:
    st.session_state.p4_start_date = datetime.today().date() - timedelta(days=365)
if 'p4_end_date' not in st.session_state:
    st.session_state.p4_end_date = datetime.today().date()

col_d1, col_d2 = st.columns(2)
with col_d1:
    START_DATE = st.date_input("Start Date", key="p4_start_date")
with col_d2:
    END_DATE = st.date_input("End Date", key="p4_end_date", max_value=datetime.today().date())
